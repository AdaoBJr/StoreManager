...
const { getProductById, updateQuantityProductById } = require('../model/productsModel');
...
const validateStock = async (sales) => {
  const products = await Promise.all(sales.map((sale) => getProductById(sale.productId)));
